...
# =============================================================================
def piston_disp(theta, Rc: float) -> np.ndarray:
    """y(theta): deslocamento do pistão a partir do PMS [m].

    y = l + r - r*cos(theta) - sqrt(l² - r² sin²(theta))
    """
    return L_ROD + R_CRANK - R_CRANK * np.cos(theta) - np.sqrt(
        L_ROD**2 - R_CRANK**2 * np.sin(theta) ** 2
    )


def cylinder_volume(theta, Rc: float) -> np.ndarray:
    ...
    return VD / (Rc - 1.0) + (VD / 2.0) * (
        ...
        + 1.0
        - np.cos(theta)
        - np.sqrt(R_ROD_RATIO**2 - np.sin(theta) ** 2)
    )


def dV_dtheta(theta, Rc: float) -> np.ndarray:
    """dV/dtheta [m³/rad]."""
    return (VD * np.sin(theta) / 2.0) * (
        1.0 + np.cos(theta) / np.sqrt(R_ROD_RATIO**2 - np.sin(theta) ** 2)
    )


def heat_area(theta, Rc: float) -> np.ndarray:
    """As(theta): área de transferência de calor [m²].

    As = 2*pi*(d/2)² + pi*d*y(theta) + pi*d*s/(Rc-1)
    """
    return (
        2.0 * np.pi * (D_BORE / 2.0) ** 2
        + np.pi * D_BORE * piston_disp(theta, Rc)
        + np.pi * D_BORE * S_STROKE / (Rc - 1.0)
    )
...
def burned_fraction(theta, theta0: float, delta_theta: float, m: float):
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        ...
        expo = np.exp(-A_WIEBE * znp1)
        x = np.where(active, 1.0 - expo, 0.0)
        dx = np.where(active, A_WIEBE * (m + 1.0) / delta_theta * zn * expo, 0.0)
    return x, dx
...
def heat_release(theta, theta0: float, delta_theta: float, m: float):
    ...
    return M_COMB * PCI * x, M_COMB * PCI * dx


# =============================================================================
# 4. Correlação de Hohenberg
# =============================================================================
def hohenberg_h(theta, P_kPa, Tg, Rc: float):
...
def _make_rhs(Rc: float, theta0: float, delta_theta: float, m: float,
    ...
    def rhs(theta: float, y: np.ndarray) -> np.ndarray:
        P, Tg, _ = y
        V = cylinder_volume(theta, Rc)
        dV = dV_dtheta(theta, Rc)
        _, dQ = heat_release(theta, theta0, delta_theta, m)   # kJ/rad
        h = hohenberg_h(theta, P, Tg, Rc)                     # W/(m² K)
        As = heat_area(theta, Rc)
        dQp = h * As * (Tg - TW) / (2.0 * np.pi * OMEGA_REV_S)  # J/rad
...
def simulate_full(
    ...
    x, dx = burned_fraction(theta_exp, theta0, delta_theta, m)
    return ModelResult(
